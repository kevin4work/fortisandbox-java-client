...
public class JsonRpcHttpClient extends JsonRpcClient {
	...
	public Object invoke(String session, String methodName, Object argument, Type returnType, Map<String, String> extraHeaders) throws Throwable {
		...
		try {
			if (this.gzipRequests) {
				...
				try (GZIPOutputStream gos = new GZIPOutputStream(baos)) {
					super.invoke(session, methodName, argument, gos);
				}
				...
				try (OutputStream send = connection.getOutputStream()) {
					super.invoke(session, methodName, argument, send);
				}
			}
			...
			try {
				...
				try (InputStream answer = getStream(connection.getErrorStream(), useGzip)) {
					...
					throw new Exception(new String(errorText, UTF_8), ef);
				}
			}
		} finally {
	...
	public <T> T invoke(String session, String methodName, Object argument, Class<T> clazz) throws Throwable {
		return (T) invoke(session, methodName, argument, Type.class.cast(clazz), new HashMap<String, String>());
	}
	
	public <T> T invoke(String session, String methodName, Object argument, Class<T> clazz, Map<String, String> extraHeaders) throws Throwable {
		return (T) invoke(session, methodName, argument, Type.class.cast(clazz), extraHeaders);
	}
